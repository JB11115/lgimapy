...
def fmt_sector_ticker_df(df, ticker_limit=16):
    df = df.reset_index().rename(columns={"index": "Ticker"})
    df = df[["Sector", "Ticker", "OAS", "OAS_prev", "OAS_change"]]
    df.rename(columns=col_fmt, inplace=True)

    ticker_df_list = []
    for sector in df["Sector"].unique():
        gdf = df[df["Sector"] == sector]
        if len(gdf) > ticker_limit:
            n = ticker_limit // 2
            gdf_top = gdf.iloc[:n]
            gdf_bottom = gdf.iloc[-n:]
            gdf = pd.concat((gdf_top, gdf_bottom))
        ticker_df_list.append(gdf)
...
def get_prev_wides(start, end, mat_range):
    ...
    prev_oas_wides = (
        prev_ix.get_value_history("OAS").max().dropna().rename("OAS_prev")
    )
...
def get_spread_change_index(start, end, mat_range):
    ...
    curr_ix.df["OAS_prev"] = curr_ix.df["Ticker"].map(prev_wides)
    curr_ix.df.dropna(axis=0, subset=["OAS_prev"], inplace=True)
    curr_ix.df["OAS_change"] = curr_ix.df["OAS"] - curr_ix.df["OAS_prev"]
    return curr_ix
...
for rating, rating_kws in ratings.items():
    ...
    for sector in sectors:
        ...
        sector_d[f"OAS_prev"].append(sector_ix.MEAN("OAS_prev").iloc[0])
        sector_ticker_df_i = (
            sector_ix.df.set_index("Ticker")[["OAS", "OAS_prev", "OAS_change"]]
    ...
    sector_df["OAS_change"] = sector_df["OAS"] - sector_df["OAS_prev"]
    ...
    dfs_d[rating]["Ticker"] = (
        rating_ix.df.set_index("Ticker")[["OAS", "OAS_prev", "OAS_change"]]
        .sort_values("OAS_change", ascending=False)
        .rename_axis(None)
    )
...
#         df.to_csv(f"{rating}_{key}_OAS_change_{fid_date}.csv")
...
doc = Document(
    f"30y_OAS_change_since_{fid_date}", path="reports/PM_meetings/2022_02"
)
doc.add_preamble(table_caption_justification="c")

col_fmt = {
    "OAS": f"OAS*{today:%-m/%d/%Y}",
    "OAS_prev": f"OAS*{prev_label}",
