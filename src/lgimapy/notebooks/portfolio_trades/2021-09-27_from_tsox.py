...
def read_tsox_df(fid_date):
    ...
    df = pd.read_csv(tsox_fid, skiprows=2, index_col=0).rename(columns=cols)
    df["CUSIP"] = df.index
    df["Size"] = df["Size"].str.replace(",", "").astype(float) * 1e3
    fill_cols = ["Side", "Long Note"]
    for col in fill_cols:
        df[col].fillna(method="ffill", inplace=True)

    return df.rename(columns=cols)[cols.values()].dropna(subset=["Size"])
...
strategy = "US Credit"
[x for x in list(acnt.df) if "Rating" in x]
# %%
def add_PT_table(
    strategy,
    accounts,
    ...
    for account_name, acnt in account_d.items():
        ...
        for _, row in df.iterrows():
            ...
            sell_d["NumericRating"].append(row["NumericRating"])
    ...
    bm_rating = (bm_df["BM_Weight"] * bm_df["NumericRating"]).sum() / bm_df[
        "BM_Weight"
    ].sum()
    avg_rating = (sell_df["MV"] * sell_df["NumericRating"]).sum() / sell_df[
        "MV"
    ].sum()
    ...
    cap = (
        ...
        f"MV Weighted Rating of Sale: {avg_rating:.2f}"
        f"(vs {bm_rating:.2f} for BM) \\\\"
    )
    if len(df) % 2 == 1:
