...
ignored_accounts = ["REDLD", "PBILD", "DCLD"]
sell_amt = 1
mv_thresh = 0
...
# %%
pt_fid = root("data/portfolio_trades/DealerSheet_20210310.csv")
pt_df = pd.read_csv(pt_fid, usecols=[4], skiprows=16).dropna().squeeze()
pt_df = pt_df[pt_df != "--"]
cusips_to_sell_old = set(pt_df)
db.load_market_data()
...
sell_list_df.to_csv(doc.path / f"{fid}.csv")
