...
def main():
    df = load_data()
    fid = "IG_30y_Spread_Dispersion"
    date = df.index[-1]
    doc = Document(fid, path="reports/dispersion", fig_dir=True, load_tex=fid)
    doc.add_preamble(
        margin={"left": 0.5, "right": 0.5, "top": 0.5, "bottom": 0.2},
        table_caption_justification="c",
        header=doc.header(
            left="Spread Dispersion",
    ...
    for section in ["MV_A", "Issuer_A", "MV_BBB", "Issuer_BBB"]:
...
def load_data():
    ...
    fid = root("data/dispersion.parquet")
    try:
        old_df = pd.read_parquet(fid)
    except (FileNotFoundError, OSError):
        # Create file from scratch.
        df = compute_stats(start="1/1/2000", end="3/1/2000")
    else:
...
def compute_stats(start, end=None):
    ...
    mv_ix = db.build_market_index(
        in_stats_index=True, maturity=(25, 32), issue_years=(None, 5)
    )
    issuer_ix = mv_ix.issuer_index()

    ixs = {"MV": mv_ix, "Issuer": issuer_ix}
    rating_kwargs = {"A": ("A+", "A-"), "BBB": ("BBB+", "BBB-")}
...
def get_percentile_table(df, date):
    ...
    subsets = {
        "_MV_A_": "A Market Value Weighted",
        "_Issuer_A_": "A Issuer Weighted",
        "_MV_BBB_": "BBB Market Value Weighted",
        "_Issuer_BBB_": "BBB Issuer Weighted",
    }
    df_list = []
...
def plot_table(df, title, doc, cbar_limits=(0, 50, 100)):
    ...
    vis.savefig(doc.fig_dir / title.replace(" ", "_"))
...
def plot_current_histograms(doc):
    ...
    for ax, (title, spreads_list) in zip(axes.flat, spread_d.items()):
        ...
        for i, (spreads, weights, color) in enumerate(
            ...
            lim = 400 if title.startswith("A") else 600
            data = data[(data["OAS"] <= lim) & (data["OAS"] > 15)]
    ...
    vis.savefig(doc.fig_dir / "current_histograms")
...
def make_section_plots(section, df, doc):
    ...
    vis.savefig(doc.fig_dir / f"{section}_timeseries")
    ...
    vis.savefig(doc.fig_dir / f"{section}_abs_jointplot")
    ...
    vis.savefig(doc.fig_dir / f"{section}_rel_jointplot")
