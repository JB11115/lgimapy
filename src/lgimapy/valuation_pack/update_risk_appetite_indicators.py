...
def update_risk_appetite_indicators(fid):
    """
    ...
    fid = "temp"
    # pca on asset prices
    df, df_pca, tab_results = run_pca_analysis()
    # calculate gs-style risk indicators
    df_risk = run_risk_indicators(df, df_pca)

    doc = Document(fid, path="valuation_pack", fig_dir=True, load_tex=False)
    # doc.start_edit("xasset_pca_tables")
    doc.add_table(
        format_table(tab_results),
...
def format_table(df):
    ...
    # get the nicely formatted names for each row
...
def run_risk_indicators(df, df_pca):
    ...
    start_date = date.today() + timedelta(days=-4500)
...
def run_pca_analysis():
    ...
    df_reg = pd.concat([df_z, df_pca], join="outer", sort=True, axis=1)
    df_reg.dropna(axis=0, how="any", inplace=True)
    ...
    tab_results = pd.DataFrame(t, index=col_names)
    mod_values = (
        tab_results["cur_level"]
        - (tab_results["cur_resid"] - tab_results["mean_resid"])
        * tab_results["std"]
    )
    tab_results = pd.concat(
        [tab_results, mod_values], join="outer", sort=True, axis=1
    )
    tab_results.rename(columns={0: "Norm_mod_val"}, inplace=True)
    ...
    tab_results["cur_resid_sign"] = [
        -1 * tab_results.loc[ix, "cur_resid"]
        if ix in asset_signs_reversed
        else tab_results.loc[ix, "cur_resid"]
        for ix in tab_results.index
    ]
    tab_results = tab_results.sort_values("cur_resid_sign")
    ...
    for key, text in plot_table.items():
        ...
        figsize = (8, 5.5)
        PCA_resid = reg_results.loc[key, "OLS_resid"]

        vis.plot_multiple_timeseries(
            [df_reg[key].rename(f"{text}"), PCA_resid.rename("PCA residual")],
            ylabel="1yr z-score",
            title=title,
            figsize=figsize,
            xtickfmt="auto",
    ...
    # c_data_gbl_chg = bdh("CECICGL", "INDEX", "PX_LAST", bbg_st_dt)
    ...
    for factor, (scalar, pca) in pca_ID.items():
        ...
        vis.plot_multiple_timeseries(
            [
                pca_frame[pca].rename(f"{pca}") * scalar,
        ...
        vis.savefig(f"{pca}_v_{factor}", path=path)
        vis.close()

    return [df, df_pca, tab_results]
...
def load_pca_bloom_data(start_date):
    ...
    df2 = pd.DataFrame(imp.fit_transform(df), index=df.index)
    df2.columns = list(df)
    warnings.simplefilter(category="ConvergenceWarning", action="default")
    return df2.fillna(method="ffill").dropna()
